...
class Client(object):
    ...
    __PACKET_IDS__ : Dict[int, Tuple[str, Type]]= {
        -1: ("unknown, PacketBase"),
         0: ("login", PacketBase), #?
         1: ("player_update", PacketBase),
         2: ("game_in_progress", PacketBase),
         3: ('sync', PacketBase),
         4: ("game_listing", PacketBase),
         5: ("recorded_game_update", PacketBase),
         6: ("histogram_update", PacketBase),
         7: ("chat_update", PacketBase),
         8: ('unknown', PacketBase),
         9: ("unknown", PacketBase),
        10: ("map_stats_update", PacketBase),
        11: ("tournament_update", PacketBase),
        12: ("win_streak", PacketBase),
        13: ("unknown", PacketBase),
        14: ("unknown", PacketBase),
        15: ("social_update", PacketBase),
        16: ("match_vods", PacketBase),
        17: ("unknown", PacketBase),
        18: ("unknown", PacketBase),
        19: ("unknown", PacketBase),
        20: ("unknown", PacketBase),
        21: ("maps", PacketBase),
        22: ("unknown", PacketBase),
    ...
    def __init__(self, **kwargs) -> None:
        ...
        kwargs:
            ...
            __collect: write any recieved packets to a directory structure.
                        Used primarily for development of the client, and
                        should not be set in consumers.

            __logger: used to set a custom logger.
        """
        from websockets.legacy.client import WebSocketClientProtocol
        self.__tsk = None
        self.__url = kwargs.get('url', Client.__DASHBOARD_API_ENDPOINT__)
        self.__collect_data = kwargs.get('collect', False)
        self.__ws: WebSocketClientProtocol = None
        self.__die: asyncio.Event = asyncio.Event()
        self.__done: asyncio.Event = asyncio.Event()
        self.__logger = kwargs.get('logger', _create_default_logger(DEBUG))
    ...
    async def run_async(self, tg: asyncio.TaskGroup | None = None) -> None:
        ...
        try:
            if tg is None:
                self.__tsk = asyncio.create_task(self.__connect())
            else:
    ...
    async def __connect(self) -> None:
        ...
        try:
            async for self.__ws in connect(self.__url):
                try:
                    while True:
                        if self.__die.is_set():
                            self.__done.set()
                            return

                        raw = await self.__ws.recv()
                        await self.__handle__message(loads(raw))
    ...
    async def __handle__message(self, msg : dict) -> None:
        ...
        """
        _cls = -1
        fn_id = f"event_unknown"
        event: Tuple[str, PacketBase] = ("event_unknown", PacketBase())
        ...
        try:
            ...
            self.__logger.debug(f"called with {msg}")
    ...
    async def event_unknown(self, packet: PacketBase) -> None:
        ...
        self.__logger.warning(f"unknown packet:")
        self.__logger.debug(f"{dumps(packet.raw, sort_keys=True, indent=2)}")
    ...
    async def event_player_update(self, packet: PacketBase) -> None:
        """
    ...
    async def event_game_in_progress(self, packet: PacketBase) -> None:
        """
        Called on cls '2'. Seems to be called when a game shows up on the dashboard
        as in progress.
        """
        pass
    ...
    async def event_game_listing(self, packet: PacketBase) -> None:
        pass
